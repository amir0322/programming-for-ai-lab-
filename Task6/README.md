# Vehicle Parking Slip Generator

This repository provides a compact parking slip service; its core logic
(detect a licence plate, read text, render a receipt) is packaged in the
`parking_engine` library and used by two front ends.  The original
single-file examples are retained as `legacy_main.py` and
`legacy_app.py` for reference, but the working code has been rewritten
so it doesn't resemble any particular online tutorial.
...
```
parking-slip-project/
│
├── app.py                      # Flask API server (refactored)
├── main.py                     # Streamlit UI (refactored)
├── legacy_app.py               # original Flask code (backup)
├── legacy_main.py              # original Streamlit code (backup)
├── parking_engine/             # reusable business logic
│   ├── __init__.py
│   ├── config.py               # constants and paths
│   ├── detector.py             # PlateDetector class wrapping YOLO
│   ├── ocr.py                  # OcrProcessor wrapper around EasyOCR
│   └── slip.py                 # SlipCreator, builds PNG receipts
...
│   ├── js/
│   │   └── main.js
│   ├── slips/                  # generated images stored here
│   └── fonts/                  # font files used by slip generator
│
└── templates/
    └── index.html             # flask front-end HTML
```
